...
class ECDDAgentCoordinator:
    ...
    Usage:
        ...
        session = coordinator.create_session(client_profile)
        
        # Step 2: After responses collected, generate assessment
        assessment, checklist = coordinator.submit_responses(session_id, responses)
        
        # Optional: Handle stakeholder queries
        answer = coordinator.answer_query(session_id, "What is the PEP status?")
        
        # Optional: Generate follow-up questions
        followup = coordinator.generate_followup(session_id, "Need more SOW details")
    """
    
    def __init__(self, project_endpoint: str = None):
        self.project_endpoint = project_endpoint
        self._questionnaire_agent = QuestionnaireGeneratorAgent(project_endpoint) if project_endpoint else QuestionnaireGeneratorAgent()
        self._reporter_agent = ReporterValidatorAgent(project_endpoint) if project_endpoint else ReporterValidatorAgent()
    ...
    def initialize(self) -> bool:
        ...
        try:
            q_init = self._questionnaire_agent.initialize()
            r_init = self._reporter_agent.initialize()
            self._initialized = q_init and r_init
            
            if self._initialized:
                logger.info("ECDDAgentCoordinator initialized successfully")
            else:
                logger.warning("ECDDAgentCoordinator partially initialized")
            
            return self._initialized
        except Exception as e:
            logger.error(f"Failed to initialize ECDDAgentCoordinator: {e}")
            return False
    ...
    def create_session(
        ...
        # Agent 1: Generate questionnaire
        questionnaire = self._questionnaire_agent.generate_questionnaire(client_profile)
        ...
        logger.info(f"Created session {session_id} with {questionnaire.get_total_questions()} questions")
    ...
    def submit_responses(
        ...
        # Agent 2: Generate assessment and checklist
        assessment, checklist = self._reporter_agent.generate_assessment(
        ...
        logger.info(f"Generated assessment for session {session_id}: {assessment.overall_risk_rating.value}")
        
        return assessment, checklist
    
    def get_ecdd_output(self, session_id: str) -> ECDDOutput:
        """
    ...
    def answer_query(
        ...
        return self._reporter_agent.answer_stakeholder_query(
            session_id=session_id,
            question=question,
            client_profile=client_profile,
    ...
    def generate_followup(
        ...
        # Agent 1: Generate follow-up questionnaire
        followup = self._questionnaire_agent.generate_followup_questionnaire(
            client_profile=client_profile,
        ...
        logger.info(f"Created follow-up session {followup_session_id}")
    ...
    def complete_review(
        ...
        session.updated_at = datetime.now(timezone.utc).isoformat()
        logger.info(f"Session {session_id} review completed: {decision}")
        
        return session
    
    def generate_followup_and_continue(
        self,
        session_id: str,
        stakeholder_feedback: str
        ...
        self._sessions[followup_session_id] = followup_session
        logger.info(f"Created follow-up session {followup_session_id} linked to {session_id}")
        
        return followup_session_id, followup
    
    def submit_followup_responses(
        self,
        ...
        # Agent 2: Generate updated assessment with combined data
        assessment, checklist = self._reporter_agent.generate_assessment(
            client_profile=client_profile,
        ...
        logger.info(f"Updated assessment for follow-up session {followup_session_id}")
    ...
    def get_latest_assessment_for_customer(
        ...
        """
        history = self.get_customer_history(customer_id, include_followups=True)
