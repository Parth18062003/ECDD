...
class DatabricksConfig:
    ...
    output_columns: Dict[str, str] = field(default_factory=lambda: {
        ...
        # Nested JSON (not "risk_assessment")
        "ecdd_assessment": "ecdd_assessment_json",
...
class DatabricksConnector:
    ...
    def _get_workspace_client(self):
        ...
        if self._workspace_client is None:
            try:
                ...
                # Use host from config or environment
                host = self.config.host
                if not host:
                    host = os.environ.get("DATABRICKS_HOST", "")

                if host:
                    # Explicit host - SDK will use CLI token for auth
                    self._workspace_client = WorkspaceClient(
                        host=host
                    )
                else:
                    # Let SDK discover from environment/profile
                    self._workspace_client = WorkspaceClient()

                logger.info(
                    f"Initialized Databricks Workspace Client (host: {host or 'auto'})")
            except ImportError:
                logger.warning(
                    "databricks-sdk not installed for volume operations")
                raise
            except Exception as e:
    ...
    def search_profiles(self, search_term: str, limit: int = 20) -> List[ClientProfile]:
        ...
        """
        id_col = self.config.input_columns.get("customer_id", "case_id")
        name_col = self.config.input_columns.get(
            "customer_name", "customer_name")
    ...
    def _write_ecdd_output_local(self, output: ECDDOutput) -> str:
        ...
        logger.info(
            f"Wrote ECDD output locally (Databricks unavailable): {filepath}")
        return output.session_id
    ...
    def write_pdf_to_volume(
        ...
        try:
            ...
            # Upload file
            # databricks-sdk expects a file-like object (seekable) for uploads.
            ...
            logger.warning(
                "databricks-sdk not installed, falling back to local storage")
    ...
    def list_session_files(self, session_id: str) -> List[str]:
        ...
        try:
            ...
            local_dir = os.path.join(
                self.config.local_fallback_path, session_id)
    ...
    def get_customer_ecdd_history(
        ...
        created_col = self.config.output_columns.get(
            "created_at", "created_at")
        ...
        try:
            ...
            for row in results:
                ...
                for json_col in ['compliance_flags_json', 'ecdd_assessment_json',
                    ...
                    if json_col in parsed and parsed[json_col]:
                        try:
                            parsed[json_col.replace('_json', '')] = json.loads(
                                parsed[json_col])
                        except:
                            pass
            ...
            logger.info(
                f"Retrieved {len(parsed_results)} ECDD records for customer {customer_id}")
    ...
    def _get_customer_history_local(
        ...
        for filename in os.listdir(self.config.local_fallback_path):
            if filename.endswith('_output.json'):
                filepath = os.path.join(
                    self.config.local_fallback_path, filename)
                try:
