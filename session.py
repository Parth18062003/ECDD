...
class ECDDSessionManager:
    ...
    def create_session(
        ...
        logger.info(f"Created session {session_id} for {client_profile.customer_name}")
        return session
    ...
    def list_sessions(self, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        try:
            files = sorted(
                ...
                key=lambda f: os.path.getmtime(os.path.join(self.storage_path, f)),
                reverse=True
            )
    ...
    def get_session_chain(self, session_id: str) -> List[QuestionnaireSession]:
        ...
        customer_sessions = self.search_sessions(customer_id=session.customer_id)
...
def get_session_manager(storage_path: str = None) -> ECDDSessionManager:
    ...
    if _session_manager_instance is None:
        _session_manager_instance = ECDDSessionManager(storage_path or SESSION_STORAGE_PATH)
